...
for (let i = 0; i < GAME_THUMBS.length; i++) {
    ...
    if (!onMobile) {
        g.addEventListener('mouseover', (_event) => {
        ...
        thumbnail.addEventListener('animationend', (_event) => {
            thumbnail.style.filter = "brightness(50%)";
        });
        g.addEventListener('mouseleave', (_event) => {
        ...
        g.addEventListener('touchstart', (_event) => {
